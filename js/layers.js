...
addLayer("w", {
    name: "wolud", // This is optional, only used in a few places, If absent it just uses the layer id.
    ...
    color: "#a14000",
    requires: new Decimal(10), // Can be a function that takes requirement increases into account
    resource: "prestigpoints", // Name of prestige currency
    baseResource: "omg", // Name of resource prestige is based on
    baseAmount() {return player.points}, // Get the current amount of baseResource
    type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
    exponent: 0.5, // Prestige currency exponent
    gainMult() { // Calculate the multiplier for main currency from bonuses
    ...
    row: 0, // Row the layer is in on the tree (0 is the first row)
    hotkeys: [
        {key: "k", description: "Reset for prestige points", onPress(){if (canReset(this.layer)) doReset(this.layer)}},
    ],
    layerShown(){return true}
})